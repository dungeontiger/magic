...
class CardTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$card = new Card("Some Card", CardType::CREATURE, new CardSubType("Goblin Wizard"), new Mana("RU"));
		assert(strcmp($card->getName(), "Some Card") == 0);
		assert($card->getType() == CardType::CREATURE);
		assert(strcmp($card->getCastingCost()->getMana(), "RU") == 0);
	...
	{
		$card = new Card("Some Card", CardType::CREATURE, new CardSubType("Goblin Wizard"), new Mana("RU"));
		$ability = new Ability(new TapCost(), new ProduceManaEffect(new Mana("R")));
